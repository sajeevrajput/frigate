...
class OpenVINOModelRunner(BaseModelRunner):
    ...
    def _async_runner(self,data) -> None:
        ...
        #         continue
            ...
            logger.info(f"[{datetime.now().timestamp():.4f}]: Time from input_store till start async inference for frame {frame_name} on connection {connection_id} in {datetime.now().timestamp() - t0:.3f}s")
